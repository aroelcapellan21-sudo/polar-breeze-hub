...
| B | Reglas Firestore abiertas: `allow read, write: if request.auth != null` | Cualquier usuario autenticado lee/escribe TODAS las colecciones. Un chofer podría borrar talonario o leer usuarios. | Escribir reglas por rol y por colección. No tocar sin plan documentado. |
| C | PIN 1234 hardcoded en `public/polar-breeze-final.html` | La app de inventario choferes sigue con PIN fijo en lugar de Firebase Auth | Plan de migración pendiente, no eliminar el PIN sin reemplazo funcionando |
