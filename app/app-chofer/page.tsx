...
export default function AppChoferPage() {
  const { user, profile, loading, logout } = useAuth();

  // Redirigir al hub si el rol no es chofer
  useEffect(() => {
    if (!loading && user && profile && profile.role !== "chofer") {
  ...
  if (loading) {
    return (
      <div
        ...
        style={{ background: "linear-gradient(135deg, #1A1A1A 0%, #2a2500 100%)" }}
      >
        <div className="text-center">
          <div
            className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center text-3xl shadow-2xl"
            style={{ background: "linear-gradient(135deg, #F5C800 33%, #D42B2B 33% 66%, #1E8C3A 66%)" }}
          >
            🧊
          </div>
          <p className="text-white font-black text-lg">App Chofer</p>
          <p className="text-[#F5C800]/80 text-sm mt-1 animate-pulse">Polar Breeze, S.R.L.</p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#1A1A1A]">
        <div className="bg-white rounded-2xl p-8 text-center shadow-xl max-w-sm mx-4">
          ...
          <p className="text-sm text-gray-500 mt-1 mb-4">Esta app es solo para Choferes.</p>
          <button
            onClick={logout}
            className="w-full py-2.5 bg-[#F5C800] text-[#1A1A1A] rounded-xl font-bold text-sm"
          >
            Salir
          </button>
        </div>
  ...
  return (
    <>
      <PWAServiceWorker scope="/app-chofer" />
      <ChoferDashboard />
    </>
  );
}
